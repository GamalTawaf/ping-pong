...
fn update_play(mut p: Play, dt: f32) -> Screen {
    let left_dir = key_dir(KeyCode::W, KeyCode::S);
    p.left = move_paddle(p.left, left_dir, PADDLE_SPEED, dt);

    match p.mode {
        Mode::VsAi => p.right = move_paddle(p.right, ai_dir(p.right, p.ball), AI_SPEED, dt),
        Mode::TwoPlayer => {
            let dir = key_dir(KeyCode::Up, KeyCode::Down);
            p.right = move_paddle(p.right, dir, PADDLE_SPEED, dt);
        }
        Mode::Solo => {}
    }
